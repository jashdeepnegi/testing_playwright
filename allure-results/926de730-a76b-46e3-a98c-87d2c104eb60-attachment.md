...
```ts
  ...
  19 |   await expect(page).toHaveTitle(/Playwright/);
  20 | });
  21 | 
...
> 27 |   await expect(page).toHaveTitle(/Playwright1/);
     |                      ^ Error: expect(page).toHaveTitle(expected) failed
  28 | 
  29 |   await page.screenshot({path: `screenshot${Date.now()}.png`, fullPage: true})
  30 | 
  31 |   // playwright logo
